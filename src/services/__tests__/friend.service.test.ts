    ...
    describe('Sistema de Búsqueda por IDs de usuario', () => {
        ...
        describe('Búsqueda de Solicitudes Pendientes. -> getPendingRequests() ', () => {
            ...
            test('Campos Incorrectos:', async () => {
                ...
                await expect(FriendService.getPendingRequests("usuarioPrueba")).rejects.toThrow();                 
            });
        });
        ...
        describe('Comprobar relacion entre 2 usuarios. -> checkRelationshipStatus() ', () => {
            ...
            test('Relación Existente:', async () => {
                ...
                const resultado = await FriendService.checkRelationshipStatus("u_1", "u_2");

                expect(resultado).toBeDefined();
                expect(resultado).toEqual(expect.stringMatching(/^(PENDING|FRIEND|BLOCKED)$/));
                
            });

            test('Relación Inexistente:', async () => {

                const resultado = await FriendService.checkRelationshipStatus("u_1", "u_15");
            ...
            test('Campos Vacios:', async () => {
                ...
                await expect(FriendService.checkRelationshipStatus("","")).rejects.toThrow(); 
                
            });

            test('Campos Incorrectos:', async () => {

                await expect(FriendService.checkRelationshipStatus("usuarioPrueba1", "usuarioPrueba2")).rejects.toThrow(); 
            });
        });
    });
    ...
    describe('Sistema de Peticiones de Amistad', () => {
        ...
        describe('Crear Peticion de Amistad -> createFriendRequest() ', () => {
            ...
            test('Usuarios Existentes:', async () => {
                ...
                const resultado = await FriendService.createFriendRequest("u_1", "u_15");
            ...
            test('Peticion Previamente Existente:', async () => {
                await expect(FriendService.createFriendRequest("u_3", "u_1")).rejects.toThrow();
            });

            test('Campos Vacios:', async () => {

                await expect(FriendService.createFriendRequest("","")).rejects.toThrow(); 
                
            });

            test('Campos Incorrectos:', async () => {

                await expect(FriendService.createFriendRequest("usuarioPrueba1", "usuarioPrueba2")).rejects.toThrow(); 
            });
        ...
        describe('Buscar Peticion de Amistad -> findRequestById() ', () => {
            ...
            test('Relación Existente:', async () => {
                ...
                for (let i = 0; i < 5 ; i++){
                    let relation = "req_" + relations_to_clean[i].id_user_1 + "_" + relations_to_clean[i].id_user_2;
        ...
        describe('Eliminar Amistad -> removeFriend() ', () => {
            ...
            test('Relacion Existente:', async () => {
                ...
                for (let i = 0; i < 5 ; i++){
                    ...
                    const resultado = await FriendService.removeFriend("u_" + relations_to_clean[i].id_user_1, "u_" + relations_to_clean[i].id_user_2 );
            ...
            test('Campos Incorrectos:', async () => {
               ...
               await expect(FriendService.removeFriend("req_1", "req_2")).rejects.toThrow(); 
