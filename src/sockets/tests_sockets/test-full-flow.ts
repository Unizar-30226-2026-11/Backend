...
function makeToken(id: string, username: string): string {
    return jwt.sign({ id, username, lobbyCode: LOBBY_CODE }, SECRET_KEY, { expiresIn: '10m' });
}

function emit(pid: string, actionType: string, payload: Record<string, unknown> = {}): void {
    const data = { lobbyCode: LOBBY_CODE, actionType, payload };
    console.log(`\n [${pid}] → client:game:action "${actionType}"`, JSON.stringify(payload));
    ...
    switch (currentPhase) {
        ...
        case 'SUBMISSION': {
            ...
            nonST.forEach((pid, i) => {
                ...
                if (!hand.length) { console.log(`⚠️  [${pid}] sin cartas para SUBMIT_CARD`); return; }
                const cardId = hand[0];
        ...
        case 'VOTING': {
            ...
            nonST.forEach((pid, i) => {
                ...
                const cardToVote = boardCards.find(c => c !== myCard) ?? boardCards[0];
                setTimeout(() => emit(pid, 'CAST_VOTE', { cardId: cardToVote }), i * 200);
            });
            break;
        }
...
function attachListeners(pid: string, allIds: string[], hostId: string): void {
    ...
    s.on('server:game:state_updated', (payload: any) => {
        ...
        if (pid === hostId) {
            ...
            if (round.boardCards?.length) console.log(`   boardCards    : [${round.boardCards.join(', ')}]`);
            if (round.playedCards) console.log(`   playedCards   : ${JSON.stringify(round.playedCards)}`);
            if (round.votes?.length) console.log(`   votes         : ${JSON.stringify(round.votes)}`);
        }
        ...
        if (pid === hostId && newPh !== prevPhase) {
            ...
            if (newPh === 'STORYTELLING' && prevPhase === 'SCORING') {
                ...
                console.log(`   Puntuaciones        : ${JSON.stringify(state.scores, null, 2)}`);
                setTimeout(() => {
...
// ─── Setup ────────────────────────────────────────────────────────────────────
async function setup(): Promise<{ hostId: string; allIds: string[]; tokens: Record<string, string> }> {
    ...
    FAKE_IDS.forEach(id => { tokens[id] = makeToken(id, id); });
    ...
    try {
        ...
        await Promise.all(allIds.map(pid =>
            new Promise<void>((resolve, reject) => {
                const s = io(BACKEND_URL, { auth: { token: tokens[pid] }, transports: ['websocket'] });
                sockets[pid] = s;
        ...
        await Promise.all(allIds.map(pid =>
            new Promise<void>(resolve => {
                if ((hands[pid] ?? []).length > 0) { resolve(); return; }
                sockets[pid].once('server:game:private_hand', () => resolve());
                setTimeout(resolve, 8000);
            })
        ));
        console.log('\n Todas las manos recibidas.');
        allIds.forEach(pid => console.log(`   [${pid}]: [${(hands[pid] ?? []).join(', ')}]`));
